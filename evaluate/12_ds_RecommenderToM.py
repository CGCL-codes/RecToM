...
def arg_parse():
    ...
    parser.add_argument("--dataset_type", type = str, default = "1_intent_rec.json")
    parser.add_argument("--model", type = str, choices = ['meta-llama/llama-3.1-8b-instruct','deepseek-r1-0528','deepseek/deepseek-r1-0528:free','deepseek/deepseek-r1-0528','deepseek-chat','gpt-4o-mini','gpt-4o','gemini-2.5-flash-lite-preview-06-17','claude-3-5-haiku-20241022'])
    parser.add_argument("--cot", action="store_true")  # 默认值为 False
...
def evaluate(args, client, problem, idx):
    ...
    if args.cot == True:
        ...
        # system_prompt = """Here is a movie recommendation dialogue, there are two agents, the RECOMMENDER and the SEEKER. The RECOMMENDER is trying to recommend movies to SEEKER. Please respond only with one or more selected options from \"A\", \"B\", \"C\", \"D\",\"E\". No other words, punctuation, or explanation."""
    ...
    # choices = problem["choice"]   # rec  


    choices = problem["choices"] # seeker
    ...
    while True:
        ...
        if args.cot != True:
            ...
            try:
                ...
                candidates = extract_answers(result, 'A-J')
